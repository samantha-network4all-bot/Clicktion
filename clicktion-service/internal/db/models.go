...
func (d *DB) GetModel(id string) (*Model, error) {
	...
	err := d.sql.QueryRow(`
		SELECT id, name, base_url, api_key, model_name,
		       is_local, is_local_override, is_default, fallback_order, created_at
		FROM models WHERE id = ? COLLATE NOCASE`, id).Scan(
...
func (d *DB) SetDefaultModel(id string) error {
	_, err := d.sql.Exec(
		`UPDATE models SET is_default = CASE WHEN id = ? COLLATE NOCASE THEN 1 ELSE 0 END`, id)
	return err
}
...
func (d *DB) DeleteModel(id string) error {
	_, err := d.sql.Exec(`DELETE FROM models WHERE id = ? COLLATE NOCASE`, id)
	return err
}
